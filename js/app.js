...
async function handleEvolutionChart() {
  ...
  try {
    ...
    const apiUrl = `https://servicodados.ibge.gov.br/api/v2/censos/nomes/${encodeURIComponent(nameInput)}`;
    ...
    console.error(error);
    alert('Erro ao buscar dados do IBGE. Tente novamente.');
  }
}
...
async function handleTop3ByUF() {
  ...
  for (let decade = 1930; decade <= 2020; decade += 10) {
    const rankingUrl = `https://servicodados.ibge.gov.br/api/v2/censos/nomes/ranking?decada=${decade}&localidade=${selectedUF}`;
...
async function handleNameComparison() {
  ...
  try {
    const compareUrl = `https://servicodados.ibge.gov.br/api/v2/censos/nomes/${encodeURIComponent(firstName)}|${encodeURIComponent(secondName)}`;
